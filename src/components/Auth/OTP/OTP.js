                ...
                <div className='middle-portion'>
                    {x === '1' ? (
                        <div className='login-heading'>
                            <p>{t('OTP.checkEmailMessage')} <span className='ques'>.</span></p>
                        </div>
                    ) : (
                        <div className='otp-heading'>
                            <p>{t('OTP.verifyEmailMessage')} <span className='ques'>.</span></p>
                        </div>
                    )}

                    <form className='input-login' onSubmit={(e) => handleSubmit(e)}>
                        <p className='Enter-otp'>{t('OTP.enterOTP')}</p>
                        ...
                        <button className='otp-verify' type='submit'>{t('OTP.verifyButton')}</button>
                        <p className='otp-head'>{t('OTP.didntReceive')}
                            {minutes === 0 && seconds === 0 ? (
                                <u onClick={handleClick}>{t('OTP.resendOTP')}</u>
                            ) : (
                                <p className='timer'>{t('OTP.timerFormat', { minutes, seconds: seconds < 10 ? `0${seconds}` : seconds })}</p>
                            )}
                        </p>
                    </form>
                </div>
